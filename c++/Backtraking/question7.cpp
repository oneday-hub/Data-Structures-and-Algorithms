// rat in maze - 3 with deadblocks in 4 directions
...
int printPath(int sr, int sc, int er, int ec, string s, vector<vector<int>> &maze) {
    ...
    int left  = printPath(sr, sc - 1, er, ec, s + "L", maze);
    int up    = printPath(sr - 1, sc, er, ec, s + "U", maze);


    // Backtrack
    maze[sr][sc] = 1;

    return rightPaths + downPaths + left + up;
}
