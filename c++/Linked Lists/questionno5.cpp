...
// Recurrsion Method:
Node* reverseL1Recursion(Node*&head){
    ...
    // recursive case
    Node* new_head = reverseL1Recursion(head->next);
    head -> next->next = head;
    head -> next = NULL; // head is now pointing to last node in reversed L1
    return new_head;
...
int main(){
    ...
    l1.display();
     ...
     l1.head = reverseL1Recursion(l1.head);
